...
class BasicClassifier(BaseEstimator):
    ...
    def __init__(self):
        ...
        if target_model == "Gaussian" :
            self.clf = GaussianNB()
            print("Load Gaussian Model")
        elif target_model == "Tree" :
            self.clf = DecisionTreeClassifier(max_depth=max_depth)
            print("Load Tree Model")
        elif target_model == "Forest" :
            self.clf = RandomForestClassifier(max_depth=max_depth, n_estimators=10)
            print("Load Forest Model")
        elif target_model == "Neightboors" :
            self.clf = KNeighborsClassifier()
            print("Load Neightboors Model")
        elif target_model == "SVM" :
            self.clf = SVC(kernel="linear")
            print("Load Linear Model")
        else :
            print(target_model, "      :        No model provided, charged Tree")
...
class UltimateClassifier(BaseEstimator):
    '''UltimateClassifier: This is a more complex example that shows how you can combine
    ...
    def __init__(self):
        ...
        self.clf = VotingClassifier(estimators=[
                    ...
                    ('Tree Classifier', DecisionTreeClassifier(max_depth=max_depth)),
                    ('Forest Classifier', RandomForestClassifier(max_depth=max_depth, n_estimators=10)),
                    ('Gaussian Classifier', GaussianNB()),
                    ('SVM Classifier', SVC(kernel="linear")),
                    ('Neighbors Classifier', KNeighborsClassifier()),
...
def test(D):  
    ...
    classifier_dict = {
            '1. UltimateClassifier': UltimateClassifier(),
